          ...
          <el-card class="test-card">
            <template #header>
              <div class="module-header">
                <span>🔐 认证模块</span>
                <el-tag :type="testResults.auth ? 'success' : 'info'">
                  {{ testResults.auth ? '✓ 已测试' : '待测试' }}
                </el-tag>
              </div>
            </template>
            <div class="test-content">
              <p>测试用户认证、登录登出功能</p>
              <el-button-group>
                <el-button @click="testAuth" :loading="testLoading.auth">
                  测试认证
                </el-button>
          ...
          <el-card class="test-card">
            ...
            <div class="test-content">
              ...
              <el-button-group>
                ...
                <el-button @click="$router.push('/ships/tracking')" type="success" plain>
                  船舶追踪
                </el-button>
              </el-button-group>
            </div>
...
import { Check, User, Ship, Document } from '@element-plus/icons-vue'
...
const testAuth = async () => {
  testLoading.auth = true
  try {
    const token = localStorage.getItem('token')
    if (token) {
      testResults.auth = true
      ElMessage.success('认证模块测试通过')
    } else {
      ElMessage.warning('未检测到登录状态')
    }
  } catch (error) {
    ElMessage.error('认证模块测试失败')
  } finally {
    testLoading.auth = false
  }
}
